...
function initTabs() {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            // 1. Cambiar clases activas en pestañas y secciones
            ...
            // 2. Gestionar los iFrames según la pestaña visible
            if (targetId === 'historical') {
                // Vaciar el grid elimina los iFrames y destruye las conexiones de streaming/audio al instante
                document.getElementById('cameraGrid').innerHTML = '';
            } else if (targetId === 'realtime') {
                // Pausar el vídeo del historial si regresa a Realtime
                const historicalVideo = document.getElementById('historicalVideo');
                if (historicalVideo) {
                    historicalVideo.pause();
                }

                // Reconstruir las cámaras en directo si la configuración ya está cargada
                if (globalConfig) {
